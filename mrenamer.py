    ...
    for path in items:
        ...
        for item in subItems:
            if item.lower() != "subs" and item.lower() != "sample":
                ...
                print(f"renaming {item} -> {newFileName}")
                os.rename(
                    os.path.join(source, path, item),
                    os.path.join(source, path, newFileName),
                )
        newFolderName = folderNameCleaner(path)
        print(f"renaming {path} -> {newFolderName}")
        os.rename(os.path.join(source, path), os.path.join(source, newFolderName))
